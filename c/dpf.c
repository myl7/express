...
uint128_t getRandomBlock() {
  ...
  if (!randKey) {
    ...
    if (!(randCtx = EVP_CIPHER_CTX_new())) printf("errors occured in creating context\n");
...
void genDPF(EVP_CIPHER_CTX *ctx, int domainSize, uint128_t index, int dataSize, uint8_t *data, unsigned char **k0,
  unsigned char **k1) {
...
#define RIGHT 1
  ...
  if (1 != EVP_EncryptInit_ex(seedCtx1, EVP_aes_128_ctr(), NULL, (uint8_t *)&s[maxLayer][1], NULL))
    printf("errors occured in init of dpf gen\n");
  if (1 != EVP_EncryptUpdate(seedCtx0, convert0, &len, zeros, dataSize)) printf("errors occured in encrypt\n");
  if (1 != EVP_EncryptUpdate(seedCtx1, convert1, &len, zeros, dataSize)) printf("errors occured in encrypt\n");
...
uint128_t evalDPF(EVP_CIPHER_CTX *ctx, unsigned char *k, uint128_t x, int dataSize, uint8_t *dataShare) {
  ...
  if (!(seedCtx = EVP_CIPHER_CTX_new())) printf("errors occured in creating context\n");
...
void clientGenProof(EVP_CIPHER_CTX *ctx, uint128_t seed, int index, uint128_t aShare, uint128_t bShare,
  uint8_t *outputsAIn, uint8_t *outputsBIn) {
...
void serverSetupProof(
  EVP_CIPHER_CTX *ctx, uint8_t *seedIn, int dbSize, uint8_t *vectorsIn, uint8_t *mIn, uint8_t *cIn) {
...
void serverComputeQuery(
  EVP_CIPHER_CTX *ctx, uint8_t *seedIn, uint8_t *mIn, uint8_t *cIn, uint8_t *proofIn, uint8_t *ansIn) {
  uint128_t *m = (uint128_t *)mIn;
  uint128_t *c = (uint128_t *)cIn;
...
// client check inputs
void clientVerify(EVP_CIPHER_CTX *ctx, uint128_t seed, int index, uint128_t aShare, uint128_t bShare, int dbLayers,
  uint8_t *bits, uint8_t *nonZeroVectorsIn) {
...
// server check inputs
void serverVerify(
  EVP_CIPHER_CTX *ctx, uint8_t *seedIn, int dbLayers, int dbSize, uint8_t *vectorsIn, uint8_t *outVectorsIn) {
  ...
  // #pragma omp declare reduction(ADDMODP: uint128_t : omp_out += omp_in + (omp_out + omp_in < omp_out ||
  // omp_out+omp_in > 0-MODP)*MODP)
  ...
  for (int i = 0; i < dbLayers; i++) {
    ...
    for (int j = 1 << (dbLayers - i - 1); j < newDbSize; j++) {
      vectorsWorkSpace[j - (1 << (dbLayers - i - 1))] =
        addModP(vectorsWorkSpace[j - (1 << (dbLayers - i - 1))], vectorsWorkSpace[j]);
    }
...
int auditorVerify(
  int dbLayers, uint8_t *bits, uint8_t *nonZeroVectorsIn, uint8_t *outVectorsAIn, uint8_t *outVectorsBIn) {
  uint128_t *nonZeroVectors = (uint128_t *)nonZeroVectorsIn;
  ...
  for (int i = 0; i < dbLayers; i++) {
    ...
    if (memcmp(&mergeAB[1 - bits[i]], &zero, 16) != 0 || (memcmp(&mergeAB[bits[i]], &nonZeroVectors[i], 16) != 0)) {
      printf("fail conditions in round %d: %d %d \n", i, memcmp(&mergeAB[1 - bits[i]], &zero, 16),
        memcmp(&mergeAB[bits[i]], &nonZeroVectors[i], 16));
      printf("auditor expected to see \n");
      print_block(nonZeroVectors[i]);
      printf("but auditor saw \n");
      print_block(mergeAB[bits[i]]);
      printf("difference \n");
      print_block(nonZeroVectors[i] - mergeAB[bits[i]]);
      print_block(mergeAB[bits[i]] - nonZeroVectors[i]);
...
void digest_message(
  const unsigned char *message, size_t message_len, unsigned char **digest, unsigned int *digest_len) {
  EVP_MD_CTX *mdctx;

  if ((mdctx = EVP_MD_CTX_create()) == NULL) printf("digest error\n");

  if (1 != EVP_DigestInit_ex(mdctx, EVP_sha256(), NULL)) printf("digest error\n");

  if (1 != EVP_DigestUpdate(mdctx, message, message_len)) printf("digest error\n");

  if ((*digest = (unsigned char *)OPENSSL_malloc(EVP_MD_size(EVP_sha256()))) == NULL) printf("digest error\n");

  if (1 != EVP_DigestFinal_ex(mdctx, *digest, digest_len)) printf("digest error\n");

  EVP_MD_CTX_destroy(mdctx);
}

void riposteClientVerify(
  EVP_CIPHER_CTX *ctx, uint128_t seed, int dbSize, uint128_t *va, uint128_t *vb, uint8_t **digestA, uint8_t **digestB) {
...
void riposteServerVerify(
  EVP_CIPHER_CTX *ctx, uint128_t seed, int dbSize, uint128_t *vector, uint128_t *mVector, uint128_t *cValue) {
...
int riposteAuditorVerify(
  uint8_t *digestA, uint8_t *digestB, uint8_t *ma, uint8_t *mb, uint128_t ca, uint128_t cb, int dbSize) {
  int pass = 1;
...
int dpf_tests() {
  ...
  EVP_CIPHER_CTX *ctx;
  if (!(ctx = EVP_CIPHER_CTX_new())) printf("errors occured in creating context\n");
  unsigned char *aeskey = (unsigned char *)"0123456789123456";
  if (1 != EVP_EncryptInit_ex(ctx, EVP_aes_128_ecb(), NULL, aeskey, NULL)) printf("errors occured in init\n");
  EVP_CIPHER_CTX_set_padding(ctx, 0);
  ...
  *seed = getRandomBlock();
  ...
  pass =
    riposteAuditorVerify(digestA, digestB, (uint8_t *)outVectorsA, (uint8_t *)outVectorsB, cValueA, cValueB, dbSize);
  ...
  // pass = riposteAuditorVerify(digestA, digestB, (uint8_t*)outVectorsA, (uint8_t*)outVectorsB, cValueA, cValueB,
  // dbSize); printf("riposte dpf check verification: %d (should be 0)\n", pass);
  ...
  *seed = getRandomBlock();
  ...
  /*
  void clientGenProof(EVP_CIPHER_CTX *ctx, uint128_t seed, int index, uint128_t aShare, uint128_t bShare, uint8_t*
  outputsAIn, uint8_t* outputsBIn); void serverSetupProof(EVP_CIPHER_CTX *ctx, uint8_t *seedIn, int dbSize, uint8_t*
  vectorsIn, uint8_t* mIn, uint8_t* cIn); void serverComputeQuery(EVP_CIPHER_CTX *ctx, uint8_t *seedIn, uint8_t* mIn,
  uint8_t* cIn, uint8_t* proofIn, uint8_t* ansIn); int serverVerifyProof(uint8_t* ans1In, uint8_t* ans2In);
  */
  ...
  for (int i = 0; i < 4; i++) {  // something went wrong at 4
    ...
    pass = 0;
    begin = clock();
    pass = riposteAuditorVerify(
      digestA, digestB, (uint8_t *)outVectorsA, (uint8_t *)outVectorsB, cValueA, cValueB, dbSizes[i]);
    elapsed = (clock() - begin) * 1000000 / CLOCKS_PER_SEC;
