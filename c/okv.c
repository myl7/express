...
int initializeServer(int numThreads) {
  for (int i = 0; i < numThreads; i++) {
    // set fixed key
    if (!(ctx[i] = EVP_CIPHER_CTX_new())) printf("errors occured in creating context\n");
    unsigned char *aeskey = (unsigned char *)"0123456789123456";
    if (1 != EVP_EncryptInit_ex(ctx[i], EVP_aes_128_ecb(), NULL, aeskey, NULL)) printf("errors occured in init\n");
    EVP_CIPHER_CTX_set_padding(ctx[i], 0);
  }

  if (!(rerandCtx = EVP_CIPHER_CTX_new())) printf("errors occured in creating context\n");
  ...
  if (1 != EVP_EncryptInit_ex(rerandCtx, EVP_aes_128_ecb(), NULL, aeskey2, NULL)) printf("errors occured in init\n");
  EVP_CIPHER_CTX_set_padding(rerandCtx, 0);

  if (!(newRowCtx = EVP_CIPHER_CTX_new())) printf("errors occured in creating context\n");
  // uint128_t aeskey2 = getRandomBlock();
  // next line just for testing. Servers would generate and share a secret key here in production
  unsigned char *aeskey3 = (unsigned char *)"3123456789123456";
  if (1 != EVP_EncryptInit_ex(newRowCtx, EVP_aes_128_ecb(), NULL, aeskey3, NULL)) printf("errors occured in init\n");
...
int processnewEntry(int dataSize, uint8_t *rowKey) {
  ...
  if (!(entry.rowKey = EVP_CIPHER_CTX_new())) printf("errors occured in creating context\n");
