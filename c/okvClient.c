...
void prepNewRow(int dataSize, uint8_t *keyA, uint8_t *keyB) {
  ...
  if (!(newRow.keyA = EVP_CIPHER_CTX_new())) printf("errors occured in creating context\n");
  ...
  if (!(newRow.keyB = EVP_CIPHER_CTX_new())) printf("errors occured in creating context\n");
...
void prepQuery(int threadNum, int localIndex, uint8_t *dataToWrite, int dataSize, int *querySize, uint8_t **dpfQueryA,
  uint8_t **dpfQueryB) {
...
void prepAudit(int threadNum, int index, uint8_t *seed, uint8_t *outputsA, uint8_t *outputsB, uint8_t *dpfQueryA,
  uint8_t *dpfQueryB) {
